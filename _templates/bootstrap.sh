...
if [ -f "${PROTOBOX_ROOT}/public/index.php" ]; then
  ...
  "slim")
    ...
    sudo sed -i "s~\s+'database'\s+=>\s+'.*',~'database' => '$PROTOBOX_NAME',~" "src/eloquent-bootstrap.php"
    sudo sed -i "s~\s+'password'\s+=>\s+'.*',~'password' => '$PROTOBOX_PASS',~" "src/eloquent-bootstrap.php"
