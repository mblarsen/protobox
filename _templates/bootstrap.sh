...
if [ -f "${PROTOBOX_ROOT}/public/index.php" ]; then
  ...
  "slim")
    ...
    sed -ri "s~('database'\s+)=>\s+'.*',~\1=> '$PROTOBOX_NAME',~" "src/eloquent-bootstrap.php"
    sed -ri "s~('password'\s+)=>\s+'.*',~\1=> '$PROTOBOX_PASS',~" "src/eloquent-bootstrap.php"
    printf "\n//Auto-generated by bootstrap.sh\nrequire __DIR__ . \"/eloquent-bootstrap.php\";" >> "src/dependencies.php"
    cp -r "$SERVER_ROOT/_templates/slim/Models" "src/"
    cp "$SERVER_ROOT/_templates/slim/composer.json" .
    cat "$SERVER_ROOT/_templates/slim/routes.php" >> src/routes.php
